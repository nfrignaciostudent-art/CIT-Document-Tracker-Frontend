...
import { ScanLine, UploadCloud, CheckCircle2, XCircle, ExternalLink } from "lucide-react";
import { Button } from "@/components/ui/button";
import { findByInternalId, getFullDisplayId, type Document } from "@/lib/mock-data";
...
function ScannerPage() {
  ...
  const handleFile = async (file: File) => {
    ...
    img.onload = () => {
      ...
      if (!code) return setResult({ ok: false });
      const raw = code.data;
      const match = raw.match(/[?&]track=([^&]+)/);
      const id = match?.[1] ?? raw;
      const doc = findByInternalId(id);
      setResult({ ok: !!doc, raw, doc: doc ?? undefined });
    };
    ...
    <div className="mx-auto max-w-5xl space-y-5">
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border bg-card p-5 shadow-[var(--shadow-soft)]">
          ...
          {!result && <p className="text-sm text-muted-foreground">No scan yet.</p>}
